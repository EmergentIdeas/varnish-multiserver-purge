...
Version: 1.0.1
Author: Dan Kolz
Author URI: https://emergentideas.com/
License: http://www.apache.org/licenses/LICENSE-2.0
Text Domain: varnish-multiserver-purge
Network: true

	Copyright 2013-2017: Dan Kolz (email: dan@emergentideas.com)

	Original Author: Leon Weidauer ( http:/www.lnwdr.de/ )
	Secondary Author: Mika A. Epstein (email: ipstenu@halfelf.org)
...
class VarnishPurger {
	...
	function varnish_rightnow_adminbar($admin_bar){
		$admin_bar->add_menu( array(
			...
			'title' => __('Empty Cache','varnish-multiserver-purge'),
	...
	public function purgeUrl($url) {
		...
		if (strpos($varniship, ',') !== FALSE) {
			foreach(explode(',', $varniship) as $theip) {
				$this->purgeUrlAtIp($url, $theip);
			}
		}
		else {
			$this->purgeUrlAtIp($url, $varniship);
		}
		
	}
	
	/**
	 * Purge URL at the IP address
	 ...
	 * @access protected
	 */
	public function purgeUrlAtIp($url, $varniship) {
		$p = parse_url($url);
